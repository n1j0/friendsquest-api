...
jest.mock('../../../../src/entities/footprintReaction.js')
...
describe('FootprintPostgresRepository', () => {
    ...
    describe('hasUserReactedToOrCreatedFootprint', () => {
        it('returns true if the footprint was created by the user', async () => {
            const id = 123
            const reactions = [] as unknown as FootprintReaction[]
            const footprint = { createdBy: { id } } as unknown as Footprint

            const result = await footprintPostgresRepository.hasUserReactedToOrCreatedFootprint(
                reactions,
                footprint,
                id,
            )
    ...
    describe('createFootprintReaction', () => {
        ...
        it('creates a footprint reaction and returns points', async () => {
            const points = {
            ...
            const hasUserReactedToOrCreatedFootprintMock = jest.fn().mockResolvedValue(false)
            footprintPostgresRepository.hasUserReactedToOrCreatedFootprint = hasUserReactedToOrCreatedFootprintMock
            const addPointsMock = jest.fn().mockReturnValue(userWithUpdatedPoints)
            ...
            expect(hasUserReactedToOrCreatedFootprintMock).toHaveBeenCalledWith(footprintReactions, footprint, user.id)
            expect(addPointsMock).toHaveBeenCalledWith(user.uid, points.FOOTPRINT_REACTION)
            expect(result).toStrictEqual({
                reaction: footprintReactionCreated,
                points: points.FOOTPRINT_REACTION,
                userPoints: userWithUpdatedPoints.points,
            })
        })

        it('creates a footprint reaction and the user doesn\'t meet the criteria to get points', async () => {
            footprintPostgresRepository.findFootprintById = jest.fn().mockResolvedValue(footprint)
            userRepository.getUserByUid = jest.fn().mockResolvedValue(user)
            ...
            // @ts-ignore
            orm.forkEm.mockImplementation(() => ({
                find: jest.fn().mockResolvedValue(footprintReactions),
                persistAndFlush: jest.fn().mockResolvedValue(true),
            }))

            footprintPostgresRepository.hasUserReactedToOrCreatedFootprint = jest.fn().mockResolvedValue(true)

            const result = await footprintPostgresRepository
                .createFootprintReaction({ id: footprint.id, message, uid: user.uid })
            expect(result).toStrictEqual({
                reaction: footprintReactionCreated,
            })
        })

        it('throws an NotFoundError if footprint does not exist', async () => {
            const error = new NotFoundError('Footprint not found')
            // @ts-ignore
            footprintPostgresRepository.findFootprintById = () => { throw error }
            await expect(footprintPostgresRepository.createFootprintReaction({ id: '', message: '', uid: '' }))
